...
df = df[df$hosp_dc_ga>44, ]

summary_strat = df[,-c(1,2)] %>%
...
From the summary table stratified by the death and tracheostomy we find that the variables measured at 36 and 44 weeks are statistically differnet and this is a sign that they are a major predictor for our composite outcome.
...
From the table of missing value proportions we can see that 55% of the surfactant variable is missing. Because of this we will drop this variable going forward.  There is also some grouping between the missing data of the 44 and 36 week data. Approximately 30% of the missing values recorded in week 44 is missing while only about 28% of the recorded values at 36 weeks is missing. Because of this I decided to fit the mixed effects model on two different datasets:
...
drop.cols = colnames(df[,colSums(is.na(df))<=138])

df %>% 
  dplyr::select(-one_of(drop.cols), Composite_Outcome) %>%
  filter(!is.na(Composite_Outcome))  %>%
  gg_miss_var(show_pct = TRUE, facet = Composite_Outcome)  + ggtitle("Missing value percentage stratified") 

```

From the table and plot above we see that the data is missing unproportinately between 36 and 44 weeks. There also seems to be more missing values  Additionally there is some pattern to the missing data when we stratify by the composite outcome. Because of this we can say that the pattern is Missing not at random. This allows us to go forward with multiple imputation.
...
df.two = subset(df.two, select = -c(Death, Trach, hosp_dc_ga, any_surf) )
...
char_columns = c(colnames(df.two)[char_columns], "Trach", "ventilation_support_level_modified.44", "ventilation_support_level.36", "del_meth","mat_ethn")
...
1) Lasso selected full model with 36 and 44 week data.
...
for (j in 1:length(BIC_vec)){
 ...
 final.re = TRUE)} else if(week_all == FALSE){
    glm1 <-  
 glmmLasso(Composite_Outcome ~ bw+ga+blength+birth_hc+prenat_ster+com_prenat_ster+mat_chorio+ gender+sga+weight_today.44+ ventilation_support_level_modified.44 + inspired_oxygen.44+p_delta.44+peep_cm_h2o_modified.44+med_ph.44,
 data= data_df,
 rnd = list(center=~1), 
 family =  binomial(link = "logit"),
 lambda = lambda[j],
...
if(week_all == TRUE){
glm_lasso =  glmmLasso(Composite_Outcome ~ bw+ga+blength+birth_hc+prenat_ster+com_prenat_ster+mat_chorio+ gender+sga+weight_today.36+ ventilation_support_level.36 + inspired_oxygen.36+p_delta.36+peep_cm_h2o_modified.36+med_ph.36+weight_today.44+ ventilation_support_level_modified.44 + inspired_oxygen.44+p_delta.44+peep_cm_h2o_modified.44+med_ph.44,
 data= data_df,
 rnd = list(center=~1), 
 family =  binomial(link = "logit"),
 lambda = lambda.final,
 final.re = TRUE)}else if(week_all ==FALSE){
   glm_lasso =  glmmLasso(Composite_Outcome ~ bw+ga+blength+birth_hc+prenat_ster+com_prenat_ster+mat_chorio+ gender+sga+weight_today.44+ ventilation_support_level_modified.44 + inspired_oxygen.44+p_delta.44+peep_cm_h2o_modified.44+med_ph.44,
 data= data_df,
 rnd = list(center=~1), 
 family =  binomial(link = "logit"),
 lambda = lambda.final,
...
four_mod.1 = glm(Composite_Outcome ~ bw+ga+blength+birth_hc+prenat_ster+com_prenat_ster+mat_chorio+ gender+sga+weight_today.44+ ventilation_support_level_modified.44 + inspired_oxygen.44+p_delta.44+peep_cm_h2o_modified.44+med_ph.44,
               data= df_train.44[[1]])
four_mod.2= glm(Composite_Outcome ~ bw+ga+blength+birth_hc+prenat_ster+com_prenat_ster+mat_chorio+ gender+sga+weight_today.44+ ventilation_support_level_modified.44 + inspired_oxygen.44+p_delta.44+peep_cm_h2o_modified.44+med_ph.44,
               data= df_train.44[[2]])
four_mod.3 = glm(Composite_Outcome ~ bw+ga+blength+birth_hc+prenat_ster+com_prenat_ster+mat_chorio+ gender+sga+weight_today.44+ ventilation_support_level_modified.44 + inspired_oxygen.44+p_delta.44+peep_cm_h2o_modified.44+med_ph.44,
               data= df_train.44[[3]])
four_mod.4 = glm(Composite_Outcome ~ bw+ga+blength+birth_hc+prenat_ster+com_prenat_ster+mat_chorio+ gender+sga+weight_today.44+ ventilation_support_level_modified.44 + inspired_oxygen.44+p_delta.44+peep_cm_h2o_modified.44+med_ph.44,
               data= df_train.44[[4]])
four_mod.5 = glm(Composite_Outcome ~ bw+ga+blength+birth_hc+prenat_ster+com_prenat_ster+mat_chorio+ gender+sga+weight_today.44+ ventilation_support_level_modified.44 + inspired_oxygen.44+p_delta.44+peep_cm_h2o_modified.44+med_ph.44,
               data= df_train.44[[5]])
...
me.44.1 = glmer(Composite_Outcome ~ bw+ga+blength+birth_hc+com_prenat_ster+  prenat_ster+mat_chorio + gender+sga+weight_today.44+weight_today.44+ ventilation_support_level_modified.44 + inspired_oxygen.44+p_delta.44+peep_cm_h2o_modified.44+med_ph.44 + (1 | center), data = df_train.44[[1]],control=glmerControl(optimizer="bobyqa"), family = binomial) 

me.44.2 = glmer(Composite_Outcome ~ bw+ga+blength+birth_hc+com_prenat_ster+  prenat_ster+mat_chorio + gender+sga+weight_today.44+weight_today.44+ ventilation_support_level_modified.44 + inspired_oxygen.44+p_delta.44+peep_cm_h2o_modified.44+med_ph.44 + (1 | center), data = df_train.44[[2]],control=glmerControl(optimizer="bobyqa"), family = binomial) 

me.44.3 = glmer(Composite_Outcome ~ bw+ga+blength+birth_hc+com_prenat_ster+  prenat_ster+mat_chorio + gender+sga+weight_today.44+weight_today.44+ ventilation_support_level_modified.44 + inspired_oxygen.44+p_delta.44+peep_cm_h2o_modified.44+med_ph.44 + (1 | center), data = df_train.44[[3]],control=glmerControl(optimizer="bobyqa"), family = binomial) 

me.44.4 = glmer(Composite_Outcome ~ bw+ga+blength+birth_hc+com_prenat_ster+  prenat_ster+mat_chorio + gender+sga+weight_today.44+weight_today.44+ ventilation_support_level_modified.44 + inspired_oxygen.44+p_delta.44+peep_cm_h2o_modified.44+med_ph.44 + (1 | center), data = df_train.44[[4]],control=glmerControl(optimizer="bobyqa"), family = binomial) 

me.44.5 = glmer(Composite_Outcome ~ bw+ga+blength+birth_hc+com_prenat_ster+  prenat_ster+mat_chorio + gender+sga+weight_today.44+weight_today.44+ ventilation_support_level_modified.44 + inspired_oxygen.44+p_delta.44+peep_cm_h2o_modified.44+med_ph.44 + (1 | center), data = df_train.44[[5]],control=glmerControl(optimizer="bobyqa"), family = binomial) 
...
coefficients_table.pres = coefficients_table

```

```{r, include=TRUE, echo = FALSE}
coefficients_table.pres = coefficients_table %>% mutate_if(is.numeric, round, digits=3)
coefficients_table.pres[,1] = c("Intercept","Birth head circumference","Prenatal Steroids(Yes)", "Birth length", "Prenatal Corticosteroids(Yes)", "Ventilation support level at 36 weeks(Non-invasive positive pressure)","Ventilation support level at 36 weeks(Invasive positive pressure)", "Fraction of Inspired Oxygen at 36 weeks", "Positive and exploratory pressure at 36 weeks", "Weight at 36 weeks", "Peak Inspiratory Pressure at 36 weeks", "Medication for Pulmonary Hypertension at 36 weeks", "Weight at 44 weeks", "Ventilation support level at 44 weeks(Non-invasive positive pressure)", "Ventilation support level at 44 weeks(Invasive positive pressure)", "Fraction of Inspired Oxygen needed at 44 weeks", "Peak Inspiratory Pressure needed at 44 weeks", "Positive and exploratory pressure at 44 weeks", "Medication for Pulmonary Hypertension at 44 weeks", "Birth weight","Obstetrical gestational age", "Maternal Chorioamnionitis(Yes)", "Gender(Male)", "Small for gestational age(Yes)")
format_zeros <- function(x) {
  ifelse(x == 0, 0, as.character(x))
}

# Apply formatting to the data frame
formatted_data <- coefficients_table.pres
formatted_data[, -1] <- lapply(formatted_data[, -1], format_zeros)

kable(formatted_data, booktabs = T) %>%
  kable_styling(position = "center", latex_options = "scale_down")


fin_coef_table = formatted_data %>%
  kable("html") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) %>%
  add_header_above(c(" ", "Backward Stepwise Selected" = 2, "Lasso Selected" = 2), bold = TRUE, background = "#888888", color = "white") %>%
  #row_spec(0, background = "#4e3629", color = "white")%>%
  row_spec(c(6:12), background = c("#4e3629"), color = "white")%>%
  row_spec(c(13:19), background = c("#888888"), color = "white")
...
Comparing time and birth variables: We find that overall, the time variable and history is a significant predictor when it comes to predicting death/trancheostomy. This is specially evident in the lasso models where none of the birth variables were selelcted and only the time variables were selected. 
...
predictions_list <- lapply(1:5, function(i) {
  predict(models[[i]], newdata = df_test[[1]], type = "response",allow.new.levels = TRUE)
})
...
predictions_list <- lapply(1:5, function(i) {
  predict(models[[i]], newdata = df_test.44[[1]], type = "response", allow.new.levels = TRUE)
})
...
#calibration
brier.lasso.44 = mean((combined_predictions.lasso.44 - (as.numeric(df_test.44[[1]]$Composite_Outcome)))^2)
...
ggplot(grid, aes(x = weight_today.44, y = predicted_prob)) +
  ...
  labs(title = "Predicted Probabilities at 44 weeks",
...
ggplot(grid, aes(x = weight_today.36, y = predicted_prob)) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE, color = "blue") +
  labs(title = "Predicted Probabilities 36 weeks",
...
From the plot above we can see that the for the 36 week data, those around the weight of 2000 are more likely to get death/trach while for those in 44 week data, those above 4000 are the most likely to get death/trach. 

# Discussion and Conclusion

In conclusion, we found that a full mixed effect model is best for prediction for the outcome of death and trachoestomy. Additionally, implementing a full model with both the 36 week and 44 week data allowed us to compare the coefficeints between 36 and 44 week data. We found that the variables describing the fraction of inspired Oxygen, medication for Pulmonary Hypertension(PH) and Peak Inspiratory Pressure (cmH2O) has opposite effects on the log odds of death/tracheostomy. We also found that the influence random effects of the centers on the log odds of the outcome accurately depict what we discussed in the EDA section.
